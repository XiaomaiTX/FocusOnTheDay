...
const state = reactive({
	_taskName: "",
	taskName: "",
	taskPriority: "text_none",
	taskPriorityName: "text_none",
	pageData: {},
});
...
Page(
	BasePage({
		onInit(params) {
			...
			state._taskName = params.task?.name || getText("text_none");
			state.taskName = params.task?.name || getText("text_none");
			state.taskPriority = params.task?.priority || "text_none";
			state.taskPriorityName = priorityMap[params.task?.priority]
				? priorityMap[params.task?.priority]
				: "text_none";
			...
			setTimeout(() => {
				...
				state.pageData = computed(() => {
					return {
						...
						items: [
							{
								title: getText("button_complete_task"),
							...
							{
								title: getText("button_task_name"),
								description:
									state.taskName || getText("text_none"),
								action: () => this.editTaskName(),
							},

							{
								title: getText("button_task_priority"),
								description: getText(state.taskPriorityName),
							...
							{
								title: getText("button_save_and_exit"),
		...
		editTaskName() {
			...
			hmUI.createKeyboard({
				...
				onComplete: (_, result) => {
					...
					state.taskName = result.data || getText("text_none");
					hmUI.deleteKeyboard();
				},
				text: state.taskName || getText("text_none"),
			});
		},
		editTaskPriority() {
			const priorityKeys = Object.keys(priorityMap);
			const priorityNames = priorityKeys.map((k) => priorityMap[k]);
			let currentIndex = priorityNames.indexOf(state.taskPriorityName);
			if (currentIndex === -1) {
				currentIndex = 0;
			}
